...
enable_audit_system() {
        ...
        # Attempt to enable the audit system
        HARDN_STATUS "info" "Attempting to enable auditd system via auditctl..."

        # Use command substitution to capture any error output
        local result
        result=$(auditctl -e 1 2>&1) || {
            HARDN_STATUS "error" "Failed to enable auditd system via auditctl: ${result}"
            HARDN_STATUS "error" "Check auditd status and configuration."
            return 1
        }
...
# Main function to configure auditd
configure_auditd() {
        ...
        # Configure specific audit rules based on STIG
        HARDN_STATUS "info" "Configuring optimized auditd rules based on STIG..."
        local audit_rules_file="/etc/audit/audit.rules"

        backup_audit_rules "$audit_rules_file"
        create_audit_rules "$audit_rules_file"
        load_audit_rules "$audit_rules_file"
