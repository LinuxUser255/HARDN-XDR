...
configure_ntp() {
        ...
        if systemctl is-active --quiet systemd-timesyncd; then
            configure_systemd_timesyncd "$ntp_servers"
        else
            configure_ntpd "$ntp_servers"
        fi
...
configure_systemd_timesyncd() {
        ...
        # Set NTP= explicitly
        update_timesyncd_servers "$temp_timesyncd_conf" "$ntp_servers"
...
configure_ntpd() {
        ...
        configure_ntp_conf "$ntp_servers"
}
...
install_ntp_package() {
        ...
        # Use && for command chaining to simplify logic
        if apt-get update >/dev/null 2>&1 && apt-get install -y ntp >/dev/null 2>&1; then
            ...
            # Set configured in parent scope
            configured=false
            return 1
        fi
}
...
# Helper function to update timesyncd servers in config file
update_timesyncd_servers() {
        local config_file="$1"
        local servers="$2"

        # Check if NTP= line exists
        if grep -qE "^\s*NTP=" "$config_file"; then
            # Replace existing NTP= line
            sed -i -E "s/^\s*NTP=.*/NTP=$servers/" "$config_file"
            return
        fi

        # Check if [Time] section exists
        if grep -q "\[Time\]" "$config_file"; then
            # Add NTP= under [Time] section
            sed -i "/\[Time\]/a NTP=$servers" "$config_file"
        else
            # Add new [Time] section with NTP=
            echo -e "\n[Time]\nNTP=$servers" >> "$config_file"
        fi
}
